 const BLOCK_STYLE = `.block_obj_wrap_div {
...
`;
    class Block_Obj {
        ...
        async init(initialization) {
            ...
            this.display = !!initialization.display;
        ...
        settingsPanel() {
            ...
            const saveButton = this.createSpanBtn('block_obj_button block_obj_save_button', '保存并关闭', '保存设置并关闭设置窗口', () => {
                this.expandWrap();
                this.saveConfig();
            });
            const onlySaveButton = this.createSpanBtn('block_obj_button block_obj_save_button', '仅保存', '仅保存设置', () => {
        ...
        createListItem(text_value) {
            ...
            delSpan.addEventListener('click', () => childSpan.remove());
        ...
        compare(str1, str2, symbol = '.', equal = false) {
            ...
            for (let i = len - 1; i >= 0; i--) {
                if (Number(arr1[i]) > Number(arr2[i])) {
                    ...
                    compareStatus = equal && Number(arr1[i]) == Number(arr2[i]);
                }
            }
